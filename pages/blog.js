...
export default function Home({allPostsData, allCategories}) {
    ...
    return (<Layout>
        ...
        <div
            className="relative flex min-h-screen w-full cursor-default content-center justify-center bg-white align-middle dark:bg-gray-900">
            <div className="relative mx-auto my-8 max-w-screen-xl px-4 py-8 md:m-8 lg:px-6 lg:py-16">
                ...
                <ul className="my-8 flex select-none flex-wrap justify-center text-center font-medium text-gray-500 dark:text-gray-400">
                    {allCategories.map((category) => (<li className="mr-2" key={category}>
                        <button
                            onClick={() => {
                                selectedCategory !== category ? setSelectedCategory(category) : setSelectedCategory("");
                            }}
                            type="button"
                            className={`mb-2 mr-2 rounded-full border border-gray-200 bg-white px-5 py-2.5 text-lg font-medium  hover:bg-gray-100 hover:text-blue-700 ${selectedCategory === category ? "z-10 text-blue-700 outline-none ring-4 ring-gray-400 dark:text-white dark:ring-gray-700" : "text-gray-900 dark:text-gray-400"}  dark:border-gray-600 dark:bg-gray-800  dark:hover:bg-gray-700 dark:hover:text-white `}
                        >
                            {category}
                        </button>
                    </li>))}
